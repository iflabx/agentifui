...
async function persistStepArtifacts(stageDir, step) {
  ...
  await writeText(
    path.join(stageDir, `${baseName}.stdout.log`),
    step.stdout || ''
  );
  await writeText(
    path.join(stageDir, `${baseName}.stderr.log`),
    step.stderr || ''
  );
  await writeJson(
...
export async function runStage(overrides = {}) {
  ...
  if (switchCommand) {
    if (dryRun) {
      switchStep = buildSkippedStep('switch-traffic', 'dry-run', switchCommand);
    } else {
  ...
  const metricsRollbackTriggered = Boolean(
    metricsStep.payload?.checks?.rollbackRequired
  );
  ...
  let ok = expectRollback
    ? rollbackValidationPassed
    : stagePassedWithoutRollback;
