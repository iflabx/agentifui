...
async function getIncrementalBoundary(sourceClient, tableName, watermarkColumn) {
  const tableRef = buildPublicTableRef(tableName)
...
async function migrateTableIncremental({
  ...
  if (!commonColumns.includes(primaryKeyColumn)) {
    throw new Error(`table ${tableName} primary key ${primaryKeyColumn} missing`)
  }
  ...
  const boundary = await getIncrementalBoundary(sourceClient, tableName, watermarkColumn)
  if (!boundary) {
    return {
      table: tableName,
      dryRun: dryRunMode,
...
async function run() {
  ...
  try {
    ...
    const skippedTables = tables.filter(table => table.status.startsWith('skipped'))
