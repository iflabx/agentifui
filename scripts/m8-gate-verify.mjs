  ...
  if (runRollout) {
    checks.push({
      ...
      env: {
        ...
        M8_STOP_AFTER_PERCENT:
          process.env.M8_GATE_ROLLOUT_STOP_AFTER_PERCENT || '',
        M8_APPROVED:
          process.env.M8_GATE_APPROVED || process.env.M8_APPROVED || '0',
        M8_REQUIRE_SWITCH_COMMAND: process.env.M8_REQUIRE_SWITCH_COMMAND || '1',
  ...
  if (runRollbackDrill) {
    checks.push({
      ...
      env: {
        ...
        M8_DRY_RUN:
          process.env.M8_GATE_ROLLBACK_DRY_RUN || process.env.M8_DRY_RUN || '0',
        M8_REQUIRE_SWITCH_COMMAND: process.env.M8_REQUIRE_SWITCH_COMMAND || '1',
        M8_REQUIRE_VERIFY_TRAFFIC_COMMAND:
          process.env.M8_REQUIRE_VERIFY_TRAFFIC_COMMAND || '1',
        M8_REQUIRE_SMOKE: process.env.M8_REQUIRE_SMOKE || '1',
