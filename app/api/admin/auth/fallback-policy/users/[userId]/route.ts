...
export async function PUT(_request: Request, _context?: unknown) {
  const requestId = resolveRequestId();
  return buildDisabledJson(503, requestId);
}

export async function PATCH(_request: Request, _context?: unknown) {
  const requestId = resolveRequestId();
  return buildDisabledJson(503, requestId);
}
...
export async function OPTIONS(_request: Request, _context?: unknown) {
  const requestId = resolveRequestId();
  return buildDisabledJson(503, requestId);
}
