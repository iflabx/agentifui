...
function buildDisabledResponse(status: number, requestId: string) {
  const detail = buildAppErrorDetail({
    ...
    source: 'next-api',
    requestId,
    code: 'DIFY_PROXY_NEXT_DISABLED',
    userMessage:
      'Dify proxy API is served by Fastify. Enable Fastify proxy/cutover to use this endpoint.',
    developerMessage:
      'Next.js dify proxy route is disabled after Fastify convergence.',
    retryable: false,
  });
...
function buildDisabledJson(status: number, requestId: string) {
  ...
  response.headers.set('x-agentifui-dify-handler', 'next-disabled');
  return response;
}
