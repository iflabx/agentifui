...
function buildDisabledResponse(status: number, requestId: string) {
  const detail = buildAppErrorDetail({
    ...
    source: 'next-api',
    requestId,
    code: 'INTERNAL_REALTIME_STATS_NEXT_DISABLED',
    userMessage:
      'Realtime stats API is served by Fastify. Enable Fastify proxy/cutover to use this endpoint.',
    developerMessage:
      'Next.js realtime stats route is disabled after Fastify convergence.',
    retryable: false,
  });
