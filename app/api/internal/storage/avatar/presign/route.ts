...
function buildDisabledResponse(status: number, requestId: string) {
  const detail = buildAppErrorDetail({
    ...
    source: 'next-api',
    requestId,
    code: 'INTERNAL_STORAGE_AVATAR_PRESIGN_NEXT_DISABLED',
    userMessage:
      'Avatar presign API is served by Fastify. Enable Fastify proxy/cutover to use this endpoint.',
    developerMessage:
      'Next.js avatar presign route is disabled after Fastify convergence.',
    retryable: false,
  });
